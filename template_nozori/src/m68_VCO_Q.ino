...
int32_t pitch_quantized, trigger;
...
inline void VCO_Q_loop_() {
  ...
  if (IN2_connect < 60) { // syncro sur le CV
    if ( (trigger == 0 ) && (audio_inR > 0xB0000000) ) { 
      trigger = 1;
      if (CV1_connect < 60) {
        tmpS = CV_filter16_out[index_filter_cv1];
        tmpS -= CV1_0V;
        tmpS *= CV1_1V;
        tmpS &= 0xFFF00000; // quantisation
        pitch_quantized = tmpS;        
      } else { // pas de CV mais un trigger qd meme : generation de notes aleatoires
        tmpS = random32();
        tmpS %= 12; // limitation sur une octave
        tmpS <<= 20; // 20 bit d'interpolation entre les notes
        pitch_quantized = tmpS;        
      }
    }
    if ( (trigger == 1 ) && (audio_inR < 0xA0000000) ) { 
      trigger = 0;
    }
  } else { // pas de syncro sur le CV 
    if (CV1_connect < 60) {
      tmpS = CV_filter16_out[index_filter_cv1];
      tmpS -= CV1_0V;
      tmpS *= CV1_1V;
